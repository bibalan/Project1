...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	vector< vector <float> > newGrid(height, std::vector <float>(width, (float)1.0/(float)(height*width)));
	...
	float p_miss) 
{
	...
	for (i = 0; i < height; i++)
	{
		for (j = 0; j < width; j++)
		{
			bool tmp_bool = (grid[i][j] == color);
			newGrid[i][j] = beliefs[i][j]*(tmp_bool*p_hit + (1 - tmp_bool)*p_miss);
		}
	}
